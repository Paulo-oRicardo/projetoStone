...
async function UpdateCoinsUser(id: string, coins: number) {
  const usersRepository = getCustomRepository(UsersRepository);
  const user = await usersRepository.findOne({ user_id: id });
  if (coins > user.coins) {
    throw new Error(
      "Usuário não possui moedas suficientes para esta transação"
    );
  }
  coins = user.coins - coins;
  usersRepository.merge(user, { coins });
  await usersRepository.save(user);
}
async function UpdateCoinsFundraising(id: string, coins: number) {
  const fundraisingRepository = getCustomRepository(FundraisingRepository);
  const fundraising = await fundraisingRepository.findOne({ id });
